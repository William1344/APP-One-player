...
export default function ViewP({route}){
    ...
    function retornaImg(){
      if(player.img == null) return assets.play_lg;
      else return route.params.player.img;
    }
    ...
    async function createFormData(photo){
      ...
      data.append("photo", {
        ...
        name  : 
          photo.fileName || 
          photo.uri.substring(photo.uri.lastIndexOf('/') + 1)
      });
      return data;
    }

    async function SalvarImgUser(result){
      const data = await createFormData(result);
      console.log("Data -> \n", data);
